...
enum L {
    ...
    private static let table: [String: [AppLanguage: String]] = [
        ...
        "perm.repair.title": [.ro: "Permisiunile trebuie reînregistrate",
                              .en: "Permissions need to be re-registered",
                              .es: "Hay que volver a registrar los permisos"],
        "perm.repair.body": [.ro: "Sistemul nu mai recunoaște permisiunea acordată anterior, chiar dacă în Setări apare încă bifată. Se întâmplă când aplicația e actualizată cu o semnătură diferită: macOS păstrează o intrare veche care nu se mai potrivește, așa că întreabă din nou la fiecare pornire.\n\n„Repară automat” șterge intrarea veche și deschide Setările, unde mai e nevoie o singură dată de bifă. După asta rămâne acordată permanent.",
                             .en: "The system no longer recognizes the permission you granted earlier, even though Settings still shows it ticked. This happens when the app is updated with a different signature: macOS keeps an old entry that no longer matches, so it asks again on every launch.\n\n“Repair automatically” removes the stale entry and opens Settings, where you tick the box once more. After that it stays granted for good.",
                             .es: "El sistema ya no reconoce el permiso que concediste antes, aunque en Ajustes siga marcado. Ocurre cuando la app se actualiza con una firma distinta: macOS conserva una entrada antigua que ya no coincide, así que vuelve a preguntar en cada inicio.\n\n«Reparar automáticamente» elimina la entrada obsoleta y abre Ajustes, donde solo hay que marcar la casilla una vez más. A partir de ahí queda concedido de forma permanente."],
        "perm.repair.menu": [.ro: "Repară permisiunile…", .en: "Repair permissions…", .es: "Reparar permisos…"],
        "perm.repair.fix": [.ro: "Repară automat", .en: "Repair automatically", .es: "Reparar automáticamente"],
        "perm.repair.settings": [.ro: "Deschide Setările", .en: "Open Settings", .es: "Abrir Ajustes"],
        "perm.repair.later": [.ro: "Mai târziu", .en: "Later", .es: "Más tarde"],
