...
enum L {
    ...
    private static let table: [String: [AppLanguage: String]] = [
        ...
        "help.zoom.body": [.ro: "Mișcă mouse-ul normal până ajungi exact unde vrei să arăți ceva, apoi ține apăsată tasta de Zoom (implicit Shift) — abia atunci apare lupa digitală, rotundă, fixă pe acel punct. Nu ține tasta cât te miști spre țintă — orientează-te normal, cu mouse-ul liber, și abia la final ții tasta. Nivelul de mărire (2x-6x) se alege din Preferințe → Zoom. Necesită permisiunea Înregistrare ecran.",
                            .en: "Move the mouse normally until you're exactly where you want to point at something, then hold the Zoom key (default Shift) — only then does the round digital loupe appear, fixed on that spot. Don't hold the key while moving toward the target — navigate normally with the mouse free, and hold the key only once you've arrived. Magnification level (2x-6x) is set in Preferences → Zoom. Requires Screen Recording permission.",
                            .es: "Mueve el ratón con normalidad hasta llegar exactamente donde quieres señalar algo, luego mantén pulsada la tecla de Zoom (por defecto Shift) — solo entonces aparece la lupa digital redonda, fija en ese punto. No mantengas la tecla mientras te desplazas hacia el objetivo — navega con normalidad, con el ratón libre, y mantén la tecla solo al llegar. El nivel de ampliación (2x-6x) se elige en Preferencias → Zoom. Requiere el permiso de Grabación de pantalla."],
        ...
        "prefs.zoomLevel.hint": [.ro: "Cât de mult mărește lupa. Mișcă mouse-ul normal până ajungi exact unde vrei, apoi ține tasta de Zoom — abia atunci apare lupa, fixă pe punctul respectiv.",
                                  .en: "How strongly the loupe magnifies. Move the mouse normally until you're exactly where you want, then hold the Zoom key — only then does the loupe appear, fixed on that spot.",
                                  .es: "Cuánto amplía la lupa. Mueve el ratón con normalidad hasta llegar exactamente donde quieres, luego mantén la tecla de Zoom — solo entonces aparece la lupa, fija en ese punto."],
